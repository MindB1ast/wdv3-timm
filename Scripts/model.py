...
def load_yolo_model(model_path: str, yolo_model_dir: Path) -> YOLO:
    ...
    Returns:
        Loaded YOLO model
    """
    print(f"\nЗагрузка модели YOLO: {model_path}")
    print(f"Директория моделей YOLO: {yolo_model_dir}")

    # First check if this is an absolute path
    if os.path.isfile(model_path):
        print(f"Загрузка модели YOLO из указанного пути: {model_path}")
        model = YOLO(model_path)
        print(f"Модель загружена: {model.model.task if hasattr(model, 'model') else 'unknown task'}")
        return model

    # Check if the model is in the specified yolo_model_dir
    local_model_path = yolo_model_dir / model_path
    if local_model_path.exists():
        print(f"Загрузка модели YOLO из локальной директории: {local_model_path}")
